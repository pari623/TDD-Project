        ...
        - generic [ref=f2e87]:
          ...
          - generic [ref=f2e91]:
            - generic [ref=f2e92]:
              ...
              - textbox "E-Mail Address" [ref=f2e94]: mtest@123.com
            - generic [ref=f2e95]:
              - generic [ref=f2e96]: Password
              - textbox "Password" [ref=f2e97]: test@123212
...
```ts
  ...
  26 |     await homePage.clickLoginLink();
  27 |     await loginPage.login(data.email,data.password);
  28 |     if(data.expected.toLowerCase()==='success')
  29 |     {
  30 |         const isLoggedIn = await myAccountPage.isMyAccountPageExist()
  31 |         expect(isLoggedIn).toBeTruthy();
  32 |     }
  33 |     else
  34 |     {
  35 |         const errorMsg = loginPage.getLoginErrorMsg();
> 36 |         expect(errorMsg).toBe("Warning: No match for E-Mail Address and/or Password.")
     |                          ^ Error: expect(received).toBe(expected) // Object.is equality
  37 |     }
  38 | 
  39 | })  
